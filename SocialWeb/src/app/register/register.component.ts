...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.registerForm.get('contactNumberWork').valueChanges
      .subscribe(e => {
        ...
        if (e < 1000000000) {
          this.contactErrorMessage = "Contact Number can't be smaller than 10 length";
          this.contactError = true;
          return
        };

        if (e >= 10000000000) {
          this.contactErrorMessage = "Contact Number can't be greater than 10 length";
          this.contactError = true;
          return
        };
    ...
    this.registerForm.get('contactNumberPersonal').valueChanges
      .subscribe(e => {
        if (e < 1000000000) {
          this.contactPersonalErrorMessage = "Contact Number can't be smaller than 10 length";
          this.contactPersonalError = true;
          return
        };

        if (e >= 10000000000) {
          this.contactPersonalErrorMessage = "Contact Number can't be greater than 10 length";
          this.contactPersonalError = true;
          return
        };
        console.log(e);
  ...
  register(e) {
    e.preventDefault();
    this.submitted = true;
    if (this.registerForm.valid || this.isDisabled) {
      // if (true) {
      this.registerForm.controls['empId'].enable()
      this.registerForm.controls['gender'].enable()
      this.registerForm.controls['dateOfBirth'].enable()
      if (this.isDisabled) {
        this.registerForm.controls['confirmPassword'].setValue(this.registerForm.get('password').value)
      }
      if (this.resume) {
